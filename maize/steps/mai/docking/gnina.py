...
class GNINAEnsemble(_GNINA):
    ...
    def run(self) -> None:
        ...
        for i, conf in enumerate(protein_confs):
            ...
            # Use a custom trained CNN model for scoring instead of builtins
            if self.cnn_model.is_set:
...
class TestSuiteGNINA:
    ...
    @pytest.mark.needs_node("gnina")
    def test_GNINA_custom_model(self, temp_working_dir: Path, protein_path: Path, test_config: Config, cnn_model_path: Path) -> None:
        """Test GNINA in isolation"""
        rig = TestRig(GNINA, config=test_config)
        params = {
            "search_center": (3.3, 11.5, 24.8),
