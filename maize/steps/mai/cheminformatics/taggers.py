...
class TagAgg(Node):
    ...
    expressions: Parameter[dict[str, tuple[str | list[str], _ValidOpsType]]] = (
        Parameter()
    )
    """
...
class TestSuiteTaggers:
    ...
    def test_RemoveTags(self, tagged_mols: list[IsomerCollection]) -> None:
        ...
        res = rig.setup_run(
            inputs={"inp": [tagged_mols]}, parameters={"tags_to_remove": ["score"]}
        )
        mols = res["out"].get()
        assert mols is not None
        assert len(mols) == 2
    ...
    def test_RemoveTags_bad(self, tagged_mols: list[IsomerCollection]) -> None:
        ...
        with pytest.raises(ParameterException):
            rig.setup_run(
                inputs={"inp": [tagged_mols]}, parameters={"tags_to_keep": ["foo-score"], "tags_to_remove": ["score"]}
            )

    def test_RenameTag(self, tagged_mols: list[IsomerCollection]) -> None:
        rig = TestRig(RenameTag)
        res = rig.setup_run(
            inputs={"inp": [tagged_mols]}, parameters={"mapping": {"foo-score": "foo-new", "score": "foo-score"}}
        )
        mols = res["out"].get()
        assert mols is not None
        assert len(mols) == 2
    ...
    def test_RenameTag_overwrite(self, tagged_mols: list[IsomerCollection]) -> None:
        rig = TestRig(RenameTag)
        res = rig.setup_run(
            inputs={"inp": [tagged_mols]}, parameters={"mapping": {"foo-score": "foo-new", "score": "foo-score"}, "overwrite": True}
        )
        mols = res["out"].get()
        assert mols is not None
        assert len(mols) == 2
