...
class Crest(Node):
    ...
    calc_name: Parameter[str] = Parameter(default="Crest_calculation")
    ...
    dump: Parameter[str] = Parameter(default="/projects/mai/projects/reactivity_preds/test_maize/local_scratch/crest_scratch")
        ...
        for i, mol in enumerate(isomercollection_list):
            ...
            for j, isomer in enumerate(mol.molecules):
                ...
                keywords = (
                    ...
                    f"-c {str(int(isomer_charge))} -u {str(int(isomer_mult - 1))} --alpb {str(self.solvent.value)} --squick --ewin 20.0 --ethr 0.15 --noopt"
                )
                if isomer.has_tag("constraints"):
...
class ButinaClust(Node):
    ...
    node_label: Parameter[str] = Parameter(default="Butina_clustering")
    """Name for the job to be used in the logging."""
    ...
    def run(self) -> None:
        ...
        for isomercollection in isomercollections:         
            for i, isomer in enumerate(isomercollection.molecules):
                self.logger.info(f"Isomer {i} conformers before filtering: {len(isomer.conformers)}")
                ...
                if isomer.has_tag('XTB_geometries'):
                    ...
                    for index in conf_ids:
                        ...
                        if isinstance(conf_tag, str):
                            self.logger.info(f"Conformer calculation failed for: conformer {index}. Skipping to the next one.")
                            continue

                        else:
                            conf_structure = Loader.molecule_from_json(conf_tag, f"conformer-{index}")
                ...
                for j in range(0,5):
                    ...
                    for i in range(j,len(mol.GetConformers())):
                        ...
                        if delt>maxd:
                            ...
                            maxi = i
                            maxj = j
                # self.logger.info("Or am I stuck here?")
                ...
                clusts: list[list[int]] = butina_clust.ClusterData(dmat, len(mol.GetConformers()), threshold, isDistData=True, reordering=True) # type: ignore
                centroids = [x[0] for x in clusts[:3]]
                self.logger.info(f"Selected conformers number {', '.join(str(p) for p in centroids) }, for isomer {isomer.name}")
                
                if isomer.has_tag('XTB_geometries'):
                    # self.logger.info("Or here?")
                    centroid_selection = {}
                    ...
                    self.logger.info(f"Isomer conformers after filtering: {len(centroid_selection)}")
