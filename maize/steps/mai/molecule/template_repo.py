...
def remove_leaving_group(smiles_string : str , smarts_pattern: str, idx_at_pt: int) -> tuple[Structure, int]:
...
def orient_substituent(
    ...
    new_temp_array = np.array(
        [coord.coords for coord in new_temp], dtype=np.float32
    )
    ...
    if magnitude != 0:
        ...
        norm_cross = np.array([0, 0, 0], dtype=np.float32)
...
def make_intermediate(
    ...
    lig_coord_attach_pt = lig_coord_dict[smarts_id(ligand_coord, "[OX2H]")] #[CX3]=[OX1] [OX2H]
...
class NiCatCycle_CC:
    ...
    def __init__(self, id: str, rsmi: str, roles: str, logger: logging.Logger) -> None:
        """
        The constructor defines the roles for the reaction. These are for
        now specifically tailored for this particular reaction mechanism.
