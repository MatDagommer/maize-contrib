...
def make_intermediate(
    ...
    lig_coord_attach_pt = lig_coord_dict[smarts_id(ligand_coord, "[CX3]=[OX1]")]
...
class ReactionControl(Node):
    ...
    def run(self) -> None:
        ...
        if hasattr(self, 'others'):
            reaction = selected_reaction(self.id, self.rsmi, roles=self.others, logger=self.logger)
        ...
        while xg16_available_result_list:
            ...
            if xg16_to_resub and m < 0:
                ...
                xg16_available_result_list = self.inp_xg16.receive()
                id_to_resub: list[str] = []
                for comps in xg16_available_result_list:
