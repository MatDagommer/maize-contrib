...
from maize.steps.mai.molecule.compchem_utils import Loader, g_Output, ConfTag
...
class Gaussian(Node):
    ...
    calc_name: Parameter[str] = Parameter(default='Gaussian_calculation')
    ...
    def _parse_gaussian_outputs(
        ...
        for i, isomer_folder in enumerate(isomer_outputs):
            ...
            for j, iso_dirname in enumerate(isomer_folder):
                ...
                isomer_g16_geometries: dict[int, list[ConfTag]] = {}
                isomer_g16_energies = {}
                ...
                if self.mode.value == "dft":
                    ...
                    for k, conf_name in enumerate(cast(list[Path], iso_dirname)):
                        ...
                        if conf_output.normal_termination():
                            self.logger.info(f"DFT calculation normally terminated for {isomer.get_tag('logging_name')}")
                            exit_code = 0
                            ...
                            try:
                                ...
                                self.logger.info(f'this is serialised geometry for conf-{k}:\n {serialised_geom}')
                            ...
                            if result_output.energy:
                                ...
                                if isomer.has_tag('isomer_free_energy_corrections'):
                                    corrections = json.loads(cast(str, isomer.get_tag('isomer_free_energy_corrections')))
                                    free_energy = float(energy) + float(corrections[str(k)])
                                    isomer_g16_free_energies[k] = free_energy

                                    if list(isomer_g16_min_energy.values())[0] >= free_energy:
                                        # conf_geometries_tag = json.loads(cast(str, isomer.get_tag("g16_dft_geometries")))
                                        isomer_g16_min_energy = {f'conf-{k}': free_energy}
                                        conf_mol = Loader.molecule_from_json(serialised_geom, f"conformer-{k}")
                                ...
                                if self.job_type.value== 'minimum':
                                    if conf_output.frequency_analysis_gaussian() == 'minimum':
                                        if isinstance(conf_output.gibbs_correction(), float):
                            ...
                            self.logger.info(f"DFT optimization failed: {isomer.get_tag('logging_name')}_conf{k}.log")
                            self.logger.info(f"Error message: {g16_error_message}")
                            isomer_g16_exit_codes[k] = 1
                            count += 1 
                            
                    isomer.set_tag(f"g16_dft_exit_codes", json.dumps(isomer_g16_exit_codes))
                    isomer.set_tag(f"g16_dft_geometries", json.dumps(isomer_g16_geometries))
                    self.logger.info(f"There are {len(isomer_g16_geometries)} conformers for {isomer.get_tag('logging_name')}")
                    self.logger.info(f"{isomer.get_tag('logging_name')} -> Conformer geometries: {isomer_g16_geometries}")    
                    
                if self.mode.value == "mm":
                    
                    self.logger.info(f'isodirname {iso_dirname}')
                    
                    
                    self.logger.info(f'isodirname {isomer}')
                    ...
                    exit_code = 1
                    self.logger.info(f'Result for {out_name}')
                    ...
                    try:
                        mm_geometry = result_output.structure_to_tag()
                        mm_serialised_geom = [mm_geometry]
                        self.logger.info(f'this is the UFF geometry for mol {out_name}:\n {serialised_geom}')
                        isomer_g16_geometries[exit_code] = mm_serialised_geom
                    ...
                    isomer.set_tag(f"g16_mm_exit_code", json.dumps(exit_code))
                    isomer.set_tag(f"g16_mm_geometries", json.dumps(isomer_g16_geometries))
                
                if isomer_g16_energies:
                    isomer.set_tag("frequency analysis", json.dumps(isomer_g16_freq_analysis))
    ...
    def run(self) -> None:
        ...
        for i, mol in enumerate(mols):
            try:
                ...
                logging_name = self.calc_name.value
                ...
                if mode_calc == 'dft':             
                    if isomer.has_tag("XTB_geometries"):
                        if isomer.has_tag("Filtered_conformers"):             
                            conf_geometries_tag = json.loads(cast(str, isomer.get_tag("Filtered_conformers")))             
                        else:
                        ...
                        for k in range(len(conf_geometries_tag)):
                            ...
                            conformer_st = Loader.molecule_from_json(conf_json, f"conformer-{k}")
                            ...
                            try:
                                conformer_st.write_xyz(conf_path / input_flname)
                            except ValueError as err:
                                self.logger.warning(
                                    "Skipping '%s' due to XYZ conversion error:\n %s", conformer_st, err
                                )
                            link = self.link.value
                            conformer_st.name = str(conf_path / input_flname)[:-4]
                            conf_g16_inp = conformer_st.export_g16_into_maize(
                                isomer_charge,
                                isomer_mult,
