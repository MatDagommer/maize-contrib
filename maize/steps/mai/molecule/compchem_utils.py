...
ConfTag = TypedDict("ConfTag", {"atoms": list[AtomEntry], "energy": float | str | None, "gradient": float | None})
...
class Structure:
    ...
    def structure_to_tag(self) -> ConfTag:
        ...
        for atom in self.atoms:
            atom_entry: AtomEntry = {'element': str(atom.label),
                        'atom_id': atom_id,
                        'coords': [float(atom.position[0]), float(atom.position[1]), float(atom.position[2])]}
...
class g_Output:
    ...
    def error_check_gaussian(self) -> str:
        ...
        with open(self.filename) as f:
            ...
            if "Error termination via Lnk1e in /usr/local/Cluster-Apps/gaussian/g09/l9999.exe" in error_portion:
                return steps_msg

            elif "Error termination via Lnk1e in /usr/local/Cluster-Apps/gaussian/g09/l1.exe" in error_portion:
                return steps_msg

            elif "Error termination via Lnk1e in /usr/local/Cluster-Apps/gaussian/g09/l103.exe" in error_portion:
                return geom_msg
            
            elif "Error termination via Lnk1e in /usr/local/Cluster-Apps/gaussian/g09/l502.exe" in error_portion:
                return geom_msg
            else:
                return generic
