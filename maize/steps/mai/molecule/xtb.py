...
class Xtb(Node):
    ...
    calc_name: Parameter[str] = Parameter(default='XTB_calculation')
            ...
            for j, iso_dirname in enumerate(mol_folder):
                ...
                if self.mode.value == 'sqm':
                    ...
                    for k, conf_name in enumerate(cast(list[Path], iso_dirname)):
                        ...
                        outfile = conf_name / f"{isomer.get_tag('logging_name')}_conf{k}_xtb_out.txt"
                ...
                if self.mode.value == 'mm':
                    ...
                    if exit_code == 0:
                        ...
                        self.logger.warning("XTB GFN-FF pre-optimisation failed for '%s'", isomer.get_tag('logging_name'))
                        isomer_gfnff_exit_codes[j] = exit_code
    ...
    def run(self) -> None:
        ...
        for i, mol in enumerate(mols):
            ...
            for j, isomer in enumerate(mol.molecules):
                ...
                self.logger.info(f"{mol.molecules[0].get_tag('logging_name')}: XTB optimisation for isomer {j}: {isomer}")
                ...
                if mode_calc == 'sqm':
                    ...
                    for k, conformer in enumerate(isomer.conformers):
                        self.logger.info(f"{mol.molecules[0].get_tag('logging_name')} - isomer {j}: XTB optimisation for conformer {k}")
